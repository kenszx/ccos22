...
## 6. Expert Agent Team Strategies (CCOS)

When the user's task benefits from multiple specialized perspectives, assemble an expert agent team automatically. Choose the right combination based on task type:

### Strategy Selection Guide

| Task Type | Agent Combination | Execution |
|-----------|------------------|-----------|
| **Code Review** | Verification + Explore × 2 (parallel) | 1 checks security, 1 checks performance, 1 checks test coverage. Run in parallel, then synthesize findings. |
| **Architecture Design** | Plan → Explore × 2 (sequential) | First spawn 2 researchers to investigate codebase, then 1 planner to design based on findings. |
| **Bug Investigation** | Explore × 3 + Verification | 3 parallel researchers test competing hypotheses. Verification validates the winning theory. Use MoA if hypotheses span different domains. |
| **New Feature** | Plan → (Explore + Verification) → General-purpose × 2 | Plan first, then research+review in parallel, finally 2 implementers work on different modules. |
| **Data Migration** | General (analysis) → General (implementation) → Verification | Sequential pipeline: analyze schema → migrate code → verify correctness. |
| **Cross-Domain Project** | MoA dispatch first (Claude+DeepSeek+Kimi) → Plan based on synthesis → Implementation per plan | When the task spans architecture + backend + frontend + testing simultaneously, start with multi-model analysis. |
| **Refactoring** | Explore (codebase survey) → General (refactor) + Verification (regression tests) | Research existing patterns, then refactor while verifying no regressions in parallel. |

### When to Use MoA (Mixture of Agents)

Use the MixtureOfAgents tool when:
- The question spans multiple domains (architecture + implementation + design)
- You need high-confidence consensus on a decision
- Different models have complementary strengths (Claude: code, DeepSeek: reasoning, Kimi: Chinese/visual)
- The user asks for "multiple perspectives" or "comprehensive analysis"

### Concurrency Rules

- **Phase 1 (Research)**: Maximum parallelism — spawn all research workers at once
- **Phase 2 (Synthesis)**: Coordinator reads findings, creates implementation spec
- **Phase 3 (Implementation)**: One worker per isolated module area, serialize on shared files
- **Phase 4 (Verification)**: Can run alongside implementation on different files

### Team Size Guidelines

- 3-5 agents is the sweet spot — balances diversity with coordination overhead
- More than 5 agents → diminishing returns (coordination costs outweigh parallelism gains)
- Single file or < 3 simple tasks → use a single agent, not a team
